...
import HexagonalAnim from "./HexagonalAnim";
...
function ScheduledHexagonalAnimPage({
  step,
  slug,
  getDoc,
  isTourOpen,
  ...
  const handleFireBaseUpload = () => {
    ...
    const uploadTask = storage
      ...
      .put(imageAsFile);
    if (edit.text != "") {
      const todoRef = firebase.database().ref("HexagonalAnim/" + edit.text);
      const todo = {
        url1: fbimg1,
        url2: fbimg2,
        url3: fbimg3,
      ...
      setpreviewlink("/scheduledlive/hexagonalanim/" + edit.text + "/" + slug);
      setloading(false);
    } else if (!livelink) {
      const todoRef = firebase.database().ref("HexagonalAnim");
      const todo = {
        url1: fbimg1,
        url2: fbimg2,
        url3: fbimg3,
    ...
    {
      edit.text != "" &&
        toast.success(" Hexagonal Animation updated successfully");
    }
  };
...
export default ScheduledHexagonalAnimPage;
