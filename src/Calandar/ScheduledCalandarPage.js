...
function ScheduledCalandarPage({
  step,
  slug,
  getDoc,
  isTourOpen,
  ...
  const handleFireBaseUpload = () => {
    ...
    if (edit.text != "") {
      ...
      const todoRef = firebase.database().ref("Calandar/" + edit.text);
      const todo = {
        url: fbimg,
        name: toname,
        handscol: handscol,
      ...
      todoRef.update(todo);
      setlivelink(
        "http://localhost:3000/scheduledlive/calandar/" + edit.text + "/" + slug
      );
      console.log(livelink, "live");
      setpreviewlink("/scheduledlive/calandar/" + edit.text + "/" + slug);

      setloading(false);
    } else if (!livelink) {
      console.log("in elseif bbb");
      const todoRef = firebase.database().ref("Calandar");
      const todo = {
        url: fbimg,
        name: toname,
        handscol: handscol,
      ...
      setlivelink(
        "http://localhost:3000/scheduledlive/calandar/" + newKey + "/" + slug
      );
      setpreviewlink("/scheduledlive/calandar/" + newKey + "/" + slug);
          ...
          var s = storage
            ...
            .then((savedImage) => {
              savedImage.ref.getDownloadURL().then((downUrl) => {
                ...
                var newKey = todoRef.push(todo).getKey();
                setlivelink(
                  "http://localhost:3000/scheduledlive/calandar/" +
                    newKey +
                    "/" +
                    slug
                );
                setpreviewlink(
                  "/scheduledlive/calandar/" + newKey + "/" + slug
                );
              });
              setloading(false);
            });
    ...
    {
      edit.text != "" && toast.success("Calandar updated successfully");
    }
  };
  ...
  async function EditPack() {
    ...
    toast.success("Calandar successfully added to your pack");
    getDoc();
  }
...
export default ScheduledCalandarPage;
